...
public class BoardController {
	...
	// Controller 에서 Service 의 메서드 호출 -> Service의 메서드에서 Dao의 메서드 호출
	// Dao의 메서드에서 Service 의 메서드로 리턴 -> Service 메서드에서 Controller 메서드로 리턴
	// 리턴 받은 내용을 model 에 싣고, ~.jsp 로 이동
	
	@Autowired
	ServletContext context; // session 을 통하지 않고, 스프링 컨테이너에 있는 빈으로 가져다 씁니다.
	...
	public String boardView_Withoutcount(Model model, HttpServletRequest request) {
		String num = request.getParameter("num");
	...
	public String board_write(Model model, HttpServletRequest request) {
		...
		// session을 거치지 않고 스프링 컨테이너에 있는 빈형태의 ServletContext를 가져다 씁니다
	...
	@RequestMapping("/boardDelectForm")
