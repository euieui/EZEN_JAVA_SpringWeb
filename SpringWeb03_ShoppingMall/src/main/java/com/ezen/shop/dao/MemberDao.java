...
public class MemberDao {
	...
	// 1. template.query 의 첫번째 전달인수로 sql 을 위치시킵니다
		...
		// 8. List 가 아닌 단일 Dto 리턴되어 야 한다면 list.get(0) 을 리턴합니다. 단, 리턴값이 null 경우 if로 처리합니다.
	...
	public int confirmID(String id) {
		...
		else result = -1; // 사용 불가능 , 검새갛ㄴ 아이디가 존재함
		return result;
	...
	public void insertMember(MemberVO mvo) {
		String sql = "insert into member2(id, pwd, name, zip_num, address, email, phone) "
				+ " values(?, ?, ?, ?, ?, ?, ?)";
	...
	public void memberUpdate(MemberVO mvo) {
		String sql = "update member2 set pwd=?, name=?, zip_num=? , address=?, "
